...
// server.get('/videogames/data', async function (req, res) {
...
server.get('/videogames', async function (req, res) {
    ...
    try {
            ...
            let  gamesId  = await  resultadoFinal.map(game => axios.get(`https://api.rawg.io/api/games/${game.id}?key=${YOUR_API_KEY}`));
